...
class CalculatorEngine:
    ...
    def operator(self, op):
        ...
        if s.phase == "entering2":
            ...
            if result is None: return self._set_error("Div by zero")
    ...
    def equals(self):
        ...
        if s.phase == "result":
            ...
            if result is None: return self._set_error("Div by zero")
            fmt = self._format(result)
            self._s = CalcState(display=fmt,history=f"{self._format(current)} {s.operator} {self._format(s.operand2)} = {fmt}",phase="result",operator=s.operator,operand1=result,operand2=s.operand2,fresh=True)
            return self._s
        ...
        if result is None: return self._set_error("Div by zero")
        fmt = self._format(result)
        self._s = CalcState(display=fmt,history=f"{self._format(s.operand1)} {s.operator} {self._format(b)} = {fmt}",phase="result",operator=s.operator,operand1=result,operand2=b,fresh=True)
        return self._s

    def clear(self):
        self._s = CalcState(); return self._s
    ...
    def backspace(self):
        ...
        if len(s.display)<=1 or (len(s.display)==2 and s.display.startswith("-")):
            ...
            self._s = CalcState(**{**s.__dict__,"display":s.display[:-1]})
        return self._s

    def sign(self):
        s = self._s
        if s.phase=="error" or s.display=="0": return s
        nd = s.display[1:] if s.display.startswith("-") else "-"+s.display
        self._s = CalcState(**{**s.__dict__,"display":nd}); return self._s
    ...
    def percent(self):
        ...
        self._s = CalcState(**{**s.__dict__,"display":self._format(result)}); return self._s
    ...
    def _set_error(self,msg):
        self._s=CalcState(display=msg,phase="error",history=self._s.history); return self._s